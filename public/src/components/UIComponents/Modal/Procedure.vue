  ...
  methods: {
    rotationEventsBus() {
      ...
      EventBus.$on('rotation', function(data) {
        ...
        this.dataAlert = {
          ...
          content: (_ => {
            let txtHtml = "<b class='text-b-ajust'>" + self.$t('modal.procedure.') + data.location + ".</b><br>"
    ...
    beforeOpened(event) {
      ...
      this.dataAlert = {
        ...
        titleCard: '<h2><b>Alerta</b></h2>',
        content: (_ => {
          let txtHtml = "<b class='text-b-ajust'>" + this.$t('modal.procedure.') + event.params.location + ".</b><br>"
