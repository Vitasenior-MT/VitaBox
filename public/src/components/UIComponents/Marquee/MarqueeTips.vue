<template>
  <p class="marquee-tips" v-html="content" :font="font + ' sans-serif'" v-bind:style="{ fontSize: font}"></p>
</template>
  ...
  export default{
    ...
    methods: {
      init() {
        ...
        _this.style.animation = animationName + ' ' + this.speed + 's' + ' linear 1'
        _this.style.webkitAnimation = animationName + ' ' + this.speed + 's' + ' linear 1'
        _this.style.mozAnimation = animationName + ' ' + this.speed + 's' + ' linear 1'
        _this.style.oAnimation = animationName + ' ' + this.speed + 's' + ' linear 1'

        _this.addEventListener("animationend", this.AnimationListener, false)
      },
      AnimationListener() {
        this.$emit('interface')
      }
