        ...
        if (expNum === 1 || parseInt(num) < 20) {
            ...
            /*if (parseInt(num[0]) !== 0) {
                if (parseInt(num) === 100) {
                    ...
                    let numTansformed = num - num[0] * Math.pow(10, (expNum - 0 - 1));
                    console.log('*******************************');
                    console.log(num);
                    console.log(num[0] * Math.pow(10, (expNum - 0 - 1)));
                    console.log(numTansformed);
                    array.push(filePathName('numbers', num[0] * Math.pow(10, (expNum - 0 - 1))));
                    if (numTansformed > 0) {
                        array.push(filePathName('numbers', 'e'));
                        prossNums(array, numTansformed);
                    }
            ...
            for (let i = 0; i < expNum; i++) {
                if (parseInt(data[0]) !== 0) {
                    if (parseInt(data) === 100) {
                        ...
                        let numTansformed = data - data[0] * Math.pow(10, (expNum - i - 1));
                        console.log('*******************************');
                        console.log(data);
                        console.log(data[0]);
                        console.log(data[0] * Math.pow(10, (expNum - i - 1)));
                        console.log(numTansformed);
                        array.push(filePathName('numbers', data[0] * Math.pow(10, (expNum - i - 1))));
                        if (numTansformed > 0) {
                            array.push(filePathName('numbers', 'e'));
                            data = numTansformed.toString();
                        }
                        if (numTansformed < 20 && numTansformed > 0) {
                            array.push(filePathName('numbers', numTansformed));
                            break;
