...
Player.prototype.processText = function (text, callback) {
    for (let index = 0 in text) {
        successlog.info(`Original Text: ${text[index]}`);
    }
    ...
    for (var index = 0; index <= arr.length - 1; index++) {
        ...
        for (var index in arr) {
            if (!fs.existsSync(arr[index])) {
                errorLog.info(`Error: ${arr[index]} file does not exists`);
            }
        }
...
function concatAll(arr, callback) {
    ...
    audioconcat(arr) // junta todos os ficheiros num único ficheiro
        .concat(dir + fileName)
