...
function prossNums(array, num) {
    ...
    console.log('prossNums------> ', num);
    if (num && expNum) {
        if (expNum === 1 || parseInt(num) < 20) {
            array.push(filePathName('numbers', num));
        } else {
            for (let i = 0; i < expNum; i++) {
                console.log('prossNums------> ', expNum);
                console.log('prossNums------> ', num[i] * Math.pow(10, (expNum - i - 1)));
                ...
                if (parseInt(num[i]) !== 0) {
                    if (parseInt(num) === 100) {
                        ...
                        if (num - num[i] * Math.pow(10, (expNum - i - 1)) < 20) {
                            array.push(filePathName('numbers', 'e'));
                            array.push(filePathName('numbers', num - num[i] * Math.pow(10, (expNum - i - 1))));
                            break;
                        }
                    }
                }
