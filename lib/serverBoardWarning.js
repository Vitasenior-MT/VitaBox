...
ServerBoardWarnings.prototype.sendData = function (data, host) {
	var self = this;
	...
	let message = data[index].sensortype + '_max:' + data[index].threshold_max_acceptable + '#confirmed';
	this.server.send(message, 0, message.length, config.port, host, function (err, bytes) {
		if (err) throw console.log('UDP ERROR');
		console.log('UDP message sent to ' + host + ':' + config.port + ' with message: ' + message);
	});
	let message01 = data[index].sensortype + '_min:' + data[index].threshold_min_acceptable + '#confirmed';
	this.server.send(message01, 0, message01.length, config.port, host, function (err, bytes) {
		if (err) throw console.log('UDP ERROR');
		console.log('UDP message sent to ' + host + ':' + config.port + ' with message: ' + message01);
	});
}
