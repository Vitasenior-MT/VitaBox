...
ServerBoardWarnings.prototype.start = function () {
	...
	this.server.on('message', function (message, remote) {
		...
		if (message.toString().includes('CHANGED')) {
			console.log('Thredhold Updated')
			let thredholdMessage = message.toString().split(' ');
			process.send({
				type: 'confirmed',
				sensortype: thredholdMessage[2].toLowerCase()
			});
		} else {
...
ServerBoardWarnings.prototype.sendData = function (message, host, callback) {
...
process.on("message", function (data, callback) {
	if (data.host) {
		sbr.sendData(data.message, data.host, callback);
	} else {
