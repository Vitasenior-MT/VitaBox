...
ServerBoardWarnings.prototype.start = function () {
	...
	this.server.on('message', function (message, remote) {
		...
		if (message.toString().includes('CHANGED')) {
			successlog.info(`-- Thredhold Updated --`);
			process.send({
				type: 'confirmed',
				sensortype: message.toString().split(' ')[2].toLowerCase()
			});
		} else {
...
ServerBoardWarnings.prototype.sendData = function (message, host) {
	this.server.send(message, 0, message.length, config.sendingHost, host, function (err, bytes) {
		if (err) errorLog.error(`UDP Error -> ${err}`);
		successlog.info(`UDP message sent to  ${host} -- ${config.sendingHost}  with message:  ${message}`);
	});
}
