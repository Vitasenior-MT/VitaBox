...
ServerBoardRegister.prototype.start = function (config, nodeIdList) {
	...
	this.server.on('message', function (message, remote) {
		successlog.info(`Register ADDRESS ---- ${remote} : ${message}`);
		message = message.replace('f80', 'f00');
		node_id = message.split(':')[message.split(':').length - 1];
		successlog.info(`Register ADDRESS ---- ${remote.address} : ${remote.port} -- NODE TO ADD -- message`);
		var nodeidRaw = message.toString().split(":");
		var nodeid = nodeidRaw[nodeidRaw.length - 1].replace(/\0[\s\S]*$/g, '');
		successlog.info(`Register NodeID ---- ${nodeid}`);
		successlog.info(`Register List ---- ${self.nodeIdList}`);
