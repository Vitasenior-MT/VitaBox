...
ServerBoardRegister.prototype.start = function (nodeIdList) {
	...
	this.server.on('message', function (message, remote) {
		message = message.toString().replace('fe80', 'fd00').replace(/\0[\s\S]*$/g, '');// faz o substitui da string recebida
		var nodeidRaw = message.toString().split(":");// divide a string pelos : guardando-a num array
		var nodeid = nodeidRaw[nodeidRaw.length - 1].replace(/\0[\s\S]*$/g, ''); // agarra na ultima posição do array e guarda numa variável
		successlog.info(`NODE List ---- ${self.nodeIdList}`);
		if (self.nodeIdList.find(nodeid)) {// procura na lista de sensores se existe o sensor a tentar registar-se
			var message1 = "flag1:" + nodeid;// cria uma variavel com uma mensagem
			self.server.send(message1, 0, message1.length, remote.port, remote.address, function (err, bytes) {// envia a mensagem em como aceita o sensor
				if (err) throw err;
				successlog.info(`NODE ACCEPTED ---- ${nodeid}`);
				successlog.info(`FLAG SENT ---- ${message1}`);
				process.send({// envia o sensor registado para ser guardado para a tabela de sensores aceites
					ip: message,
					node_id: nodeid
				});
			});
			...
			self.server.send(message1, 0, message1.length, remote.port, remote.address, function (err, bytes) { // envia a mensagem em como rejeita o sensor
				if (err) throw err;
