...
var self = module.exports = {
    ...
    loadSettings: function () {
        ...
        self.startBoardServer(() => {
            notificationManager.loadSettings(sktcon, self);
        });
    ...
    startBoardListenerRevised: function () {
        ...
        serverBoardListenerRevised.start(iptables);
    },
    /**
...
function processData(data) {
    ...
    iptables.getData({ node_id: node_id }, (result) => {
        if(result[0]){
            self.child3.send({ message: data[index].sensortype + '_max:' + data[index].threshold_max_acceptable + '#', host: result[0].node_ip });
        }
        index++;
        if (data[index]) {
            processData(data);
...
function thresholdsToUpdate() {
    index = 0;
    sensorlib.getDistictAll(null, (data) => {
        if (data[0]) {
            allBeforeUpdateSensors = data[0].sensors;
...
function processConfirmation(data) {
    updatedthresholds[data.sensortype] = data.sensortype;
    for (var index in allBeforeUpdateSensors) {
        console.log('---- ', updatedthresholds[allBeforeUpdateSensors[index].sensortype]);
    }
}