        ...
        self.updateSensorsList(() => {
            ...
            connectServerlib.getBoards((board) => {
                sensorlib.getDistictAll(null, (data) => {
                    ...
                    //self.child3.send({ thresholds: data[0].sensors });
                });
...
function processData(data) {
    ...
    iptables.getData({ node_id: node_id }, (result) => {
        console.log('-' + index + '---Min----' + data[index].threshold_min_acceptable + '-----------------------------> ', result[0].node_ip);
        console.log('-' + index + '---Max----' + data[index].threshold_max_acceptable + '-----------------------------> ', result[0].node_ip);
        console.log(data[index].sensortype + '_max:' + data[index].threshold_max_acceptable + '#confirmed');
        console.log(data[index].sensortype + '_min:' + data[index].threshold_min_acceptable + '#confirmed');
        self.child3.send({ message: data[index].sensortype + '_max:' + data[index].threshold_max_acceptable + '#confirmed', host: result[0].node_ip });
        self.child3.send({ message: data[index].sensortype + '_min:' + data[index].threshold_min_acceptable + '#confirmed', host: result[0].node_ip });
        index++;
        if (data[index]) {
            processData(data);
        }
