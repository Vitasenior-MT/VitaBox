...
function processData(data) {
    ...
    iptables.getData({ node_id: node_id }, (result) => {
        ...
        index++;
        if (data[index]) {
            processData({ message: data[index].sensortype + '_max:' + data[index].threshold_max_acceptable + '#confirmed', host: result[0].node_ip });
            processData({ message: data[index].sensortype + '_min:' + data[index].threshold_min_acceptable + '#confirmed', host: result[0].node_ip });
        }
    });
}