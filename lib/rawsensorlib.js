...
module.exports = {
  insert: function (data, sensors, callback) {
    ...
    for (var index = 0; index < data.data.length; index++) {
      let sensor = data.data[index];
      if (sensors[data.node_id]) {
        if (sensors[data.node_id].sensorID[sensor.sensortype]) {
          object.push({
            board_id: sensors[data.node_id].board_id,
            sensor_id: sensors[data.node_id].sensorID[sensor.sensortype].sensor_id,
            transducer: sensor.sensortype,
            measure: sensors[data.node_id].sensorID[sensor.sensortype].measure,
            location: sensors[data.node_id].sensorID[sensor.sensortype].location,
            datetime: data.datetime,
            value: sensor.value
          });
          sensorData.push(sensors[data.node_id].sensorID[sensor.sensortype]);
        }
      }
    }
    RawSensor.insert(object, sensorData, callback);
  },
