    ...
    patientslib.getPatientInfo(args.pacientId, args.action, res, function (pacientInfo) {
      settinglib.getFlagBandFit(null, (data) => {
        rawsensorlib.getSensorsbleIdLastData(args.pacientId + "-" + pacientInfo.mac_addr, args.action, res, function (bleLastExec) {
          ...
          let height = parseInt(pacientInfo.height.toString().split(/[.,]/g, ""))
          console.log('------------> ', height[0] + (parseInt(height[1]) < 10) ? parseInt(height[1]) * 10 : parseInt(height[1]));
            ...
            settinglib.updateFlagBandFit({ flag: true }, (result) => {
              ...
              bleChildProc.on('message', function (data) {
                if (data.proc === 'bluetooth') {
                  ...
                  // console.log(data);
                  settinglib.updateFlagBandFit({ flag: false }, (result) => { });
