...
module.exports = {
    ...
    afterGettingBoards: function (res) {
        connectServerlib.getPatients(null, () => {

        });
        connectServerlib.getBoards(null, (data) => {
            res();
        });
    },
    deleteData: function () {
    ...
    postData: function (data) {
        ...
        rawsensorlib.insert(data, sensors, (result) => {
            processSensorData(result, result.length - 1);
        });
    }
}

function processSensorData(result, index) {
    if (index >= 0) {
        rawsensorlib.updateAvg(result[index], (res) => {
            result[index].avg = res;
            sensorlib.update(result[index], null, () => {
                processSensorData(result, --index);
            });
        });
    }
}
