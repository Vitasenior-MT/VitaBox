...
var sensorlib = require('./sensorlib.js'),
    ...
    timeTillAvgOutExpiredDate = timeCalculator(5, 0, 0), // In case the configs fail the time wont be messed up
    deleteRate = timeCalculator(5, 0, 0), // In case the configs fail the time wont be messed up
    postRate = timeCalculator(0, 0, 5), // In case the configs fail the time wont be messed up
    boardid = [], // saves the list of all boards
    sensorid = []; // saves the list of all sensor types

module.exports = {
    configurations: function (configs) {
        timeTillAvgOutExpiredDate = timeCalculator(configs.timeTillAvgOutExpiredDate.hour, configs.timeTillAvgOutExpiredDate.min, configs.timeTillAvgOutExpiredDate.sec);
        deleteRate = timeCalculator(configs.deleteRate.hour, configs.deleteRate.min, configs.deleteRate.sec);
        postRate = timeCalculator(configs.postRate.hour, configs.postRate.min, configs.postRate.sec);
    },
