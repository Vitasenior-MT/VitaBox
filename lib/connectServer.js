...
ConnectServer.prototype.requestToken = function (res) {
  var self = this;
  this.httpsReq(
    this.createOptions('POST', null, '/vitabox/' + this.config.key + '/connect'),
    { password: this.config.pass }, (result) => {
      if (result.status === 200) {
        console.log('Complete Data: ', JSON.parse(result.responce))
        self.token = JSON.parse(result.responce).token;
        settinglib.postToken({
          data: {
            token: JSON.parse(result.responce).token,
            district: JSON.parse(result.responce).district,
            locality: JSON.parse(result.responce).locality
          }
        }, (data) => {
