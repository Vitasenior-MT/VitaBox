...
ConnectServer.prototype.postSettings = function (self2, res, settings) {
  //TODO: usar como referencia o serverble a forma de passar as funções
...
ConnectServer.prototype.getPatients = function (self2, res) {
  ...
  var options = self.createOptions('GET', self.token, '/vitabox/' + self.config.key + '/patient');
...
ConnectServer.prototype.getBoards = function (self2, res) {
  ...
  var options = self.createOptions('GET', self.token, '/vitabox/' + self.config.key + '/board');
  self.httpReq(options, false, (result) => {
    self.resultData((data) => {
...
ConnectServer.prototype.requestToken = function (self2, res, data) {
  var self = self2 ? self2 : this;
  let options = self.createOptions('POST', null, '/vitabox/' + self.config.key + '/connect');
  self.httpReq(options, { password: self.config.pass }, (result) => {
...
ConnectServer.prototype.resultData = function (res, result, data) {
  ...
  if (typeof res === 'function') {
    result.status === 200 ?
      ...
      : self.statusHandler(result.status, result, data);
  } else {
 ...
 * TODO:
 */
ConnectServer.prototype.statusHandler = function (statusCode, info, data) {
  ...
  switch (statusCode) {
    ...
    case 'ENOTFOUND':
      ...
      console.log('Não consegue chagar ao servidor??');
      break;
    default:
      break;
  }
...
ConnectServer.prototype.httpReq = function (options, adicionalInfo, callback, data) {
  ...
  req.on('error', function (e) {
    ...
    self.statusHandler(e.code, e, data);
  });
...
var self = module.exports = ConnectServer;