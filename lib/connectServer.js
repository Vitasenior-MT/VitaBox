 ...
 * @param {}  
 */
ConnectServer.prototype.requestToken = function (self2) {
  var self = self2 ? self2 : this;
  let data = {
    func: self.requestToken,
    method: 'RequestToken'
  };
  ...
  self.httpReq(options, { password: config.pass }, (result) => {
    self.resultData((data) => {
      ...
      settinglib.postToken({ body: self.token }, (data) => { console.log(data); });
    }, result, data);
  }, data);
}
 ...
 * @param {}  
 */
ConnectServer.prototype.resultData = function (res, result, func) {
  var self = this;
...
ConnectServer.prototype.statusHandler = function (statusCode, data) {
  ...
  switch (statusCode) {
    case 401:
      console.log('***401***');
      //Nao foi testado devido ao servidor não estar ligado
