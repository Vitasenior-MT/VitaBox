'use strict'
var ConnectServer = require('./connectServer'),
  remotelib = require('./remotelib.js'),
  boardlib = require('./boardlib.js'),
  settinglib = require('./settinglib.js'),
  request = require('request'),
  iconv = require('iconv-lite'),
  htmlparser = require("htmlparser2"),
  cheerio = require('cheerio'),
  patientslib = require('./patientslib.js'),
  utils = require('./utils.js'),
  mode = process.env.NODE_ENV || "DEV",
  config = require('./../config-' + mode.toLowerCase() + '/config.js'),
  reconnectRate = config.ReconnectRate,
  reconnect = utils.timeCalculator(reconnectRate.hour, reconnectRate.min, reconnectRate.sec),
  utils = require('./utils.js'),
  index = 0,
  logger = require('./logger'),
  errorLog = logger.errorlog,
  successlog = logger.successlog;

ConnectServer = new ConnectServer();

var self = module.exports = {
  postSettings: (req, res) => {
    remotelib.getSettings(function (err, data) {
      err ? console.log('error ', err) : ConnectServer.put((result) => {
        self.handler((code) => {
          if (code === 'newPost') {
            self.postSettings(req, res);
          } else {
            res(code);
          }
        }, result);
      }, { settings: data }, '/api/settings/vitabox');
    });
  },//remove?
  postSensorData: (req, res, data) => {
    ConnectServer.post((result) => {
      console.log('soooo: ', result, data);
      self.handler((code) => {
        if (code === 'newPost') {
          self.postSensorData(req, res, data);
        } else {
          res(code);
        }
      }, result);
    }, { records: data }, '/api/record');
  },
  confirmation: (callback) => {
    ConnectServer.put((result) => {
      self.handler((code) => {
        if (code === 'newPost') {
          self.confirmation(callback);
        } else {
          callback();
        }
      }, result);
    }, {}, '/api/notification');
  },
  getBoards: (res) => {
    ConnectServer.get((data) => {
      self.handler((code) => {
        if (code === 'newPost') {
          self.getBoards(res);
        } else {
          boardlib.postBoards(JSON.parse(data.responce), (result) => {
            res(result);
          });
        }
      }, data);
    }, '/api/vitabox/' + ConnectServer.config.key + '/board');
  },
  getPatients: (res) => {
    ConnectServer.get((data) => {
      self.handler((code) => {
        if (code === 'newPost') {
          self.getPatients(res);
        } else {
          successlog.info(`Patients data ---- ${data.responce}`);
          patientslib.postPatients(JSON.parse(data.responce), (result) => {
            res(result);
          });
        }
      }, data);
    }, '/api/vitabox/' + ConnectServer.config.key + '/patient');
  },
  getSettings: (req, res) => {
    ConnectServer.get((data) => {
      boardlib.postBoards2(JSON.parse(data.data.responce), (result) => {
        console.log(result);
      });
    }, '/api/settings/vitabox');
  },//remove?
  getWarnings: (res) => {
    ConnectServer.get((data) => {
      self.handler((code) => {
        if (code === 'newPost') {
          self.getWarnings(res);
        } else {
          res(data);
        }
      }, data);
    }, '/api/warning');
  },
  getUsers: (req, res) => {
    ConnectServer.get((data) => {
      console.log(data)
      if (data.status === 200) {
        return res.json({
          status: true,
          data: {
            data: JSON.parse(data.responce),
            vitaboxId: config.ServerConfigs.key,
            token: process.env.token,
            host: config.ServerConfigs.host,
            port: config.ServerConfigs.port,
            peerhost: config.ServerConfigs.peerhost,
            peerport: config.ServerConfigs.peerport,
            secure: config.ServerConfigs.peerport === 443 ? true : false
          }
        });
      }
    }, '/api/vitabox/' + ConnectServer.config.key + '/user');
  },
  getTempo: (req, res) => {
    if (process.env.connection) {
      settinglib.getLocationDistrict(null, (data) => {
        // let dataDL = {};
        // dataDL.district = "santarem";
        // dataDL.locality = "tomar";
        if (data[0].district) {
          let district = utils.normalyzeStringLowercase(data[0].district);
          let locality = utils.normalyzeStringLowercase(data[0].locality);
          let requestOptions = {
            encoding: null,
            method: "GET",
            uri: "https://www.weatheravenue.com/pt/europe/pt/" + district + "/" + locality + "-tempo.html"
          };
          request(requestOptions, function (error, response, body) {
            console.log(district);
            console.log(locality);
            if (error) {
              return res.json({
                status: false,
                data: ""
              });
            }
            var utf8String = iconv.decode(new Buffer(body), "UTF-8");
            // console.log("Teste --- ", utf8String);
            const $ = cheerio.load(utf8String);
            let output = [];
            res.json({
              status: true,
              data: "<table>" + $("table.weather-border").html() + "</table>"
            });
          });
        }
      });
    }
  },
  getFarmaciasServico: (req, res) => {
    if (process.env.connection) {
      settinglib.getLocationDistrict(null, (data) => {
        // let dataDL = {};
        // dataDL.district = "santarem";
        // dataDL.locality = "tomar";
        if (data[0].district) {
          let district = utils.normalyzeStringLowercase(data[0].district);
          let locality = utils.normalyzeStringLowercase(data[0].locality);
          let requestOptions = {
            encoding: null,
            method: "GET",
            uri: "http://farmaciasdeservico.net/widget/?localidade=" + district + "%7C" + locality + "&cor_fundo=%23FFFFFF&cor_titulo=%23000000&cor_texto=%23333333&margem=10&v=1"
          };

          request(requestOptions, function (error, response, body) {
            let cnt = -1,
              startPush = false,
              startInfoPush = false,
              resultFarmacias = {
                farmacias: [],
                district: data[0].district.charAt(0).toUpperCase() + data[0].district.substring(1),
                locality: data[0].locality.charAt(0).toUpperCase() + data[0].locality.substring(1),
                info: []
              };
            if (error) {
              return res.json({
                status: false,
                data: resultFarmacias
              });
            }
            var utf8String = iconv.decode(new Buffer(body), "ISO-8859-1");
            // console.log("Teste --- ", utf8String);
            var parser = new htmlparser.Parser({
              onopentag: function (name, attribs) {
                if (name === "script" && attribs.type === "text/javascript") {
                  // console.log("JS! Hooray!");
                }
                if (name === "div" && attribs.class === 'farmacia') {
                  // console.log("farmacia", attribs);
                  cnt++;
                  resultFarmacias.farmacias[cnt] = [];
                  startPush = true
                }
                if (name === "div" && attribs.class === 'infoFarmacias') {
                  // console.log("infoFarmacias", attribs);
                  startPush = false;
                  startInfoPush = true
                }
              },
              ontext: function (text) {
                text = text.toString().replace(/\r\n|\r|\n|\t/g, "");
                if (text.trim() !== "") {
                  // console.log("-->", text.trim());
                  if (startPush) {
                    resultFarmacias.farmacias[cnt].push(text.trim());
                  }
                  if (startInfoPush) {
                    resultFarmacias.info.push(text.trim());
                  }
                }
              },
              onclosetag: function (tagname) {
                if (tagname === "script") {
                  // console.log("That's it?!");
                } else if (tagname === "body") {
                  // console.log("Tag body!");
                  res.json({
                    status: true,
                    data: resultFarmacias
                  });
                }
              }
            }, { decodeEntities: true });
            parser.write(utf8String);
            parser.end();
          });
        }
      });
    }
  },
  handler: (callback, result) => {
    if (result.status !== 200) {
      setTimeout(() => {
        ConnectServer.requestToken((code) => {
          if (code === 200) {
            index = 0;
            reconnect = utils.timeCalculator(reconnectRate.hour, reconnectRate.min, reconnectRate.sec);
            callback('newPost');
          } else {
            self.handler(callback, result);
          }
        });
        index++;
        if (index > 10) {
          reconnect = utils.timeCalculator(reconnectRate.extra.hour, reconnectRate.extra.min, reconnectRate.extra.sec);
        }
      }, reconnect);
    } else {
      callback({
        status: result.status,
        data: result.data
      });
    }
  }
};