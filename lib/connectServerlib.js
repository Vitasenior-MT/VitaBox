  ...
  getUsers: (req, res) => {
    ConnectServer.get((data) => {
      ...
      if (data.responce) {
        ...
        return res.json({
          status: true,
          data: { data: JSON.parse(data.responce), vitaboxId: config.ServerConfigs.key, token: ConnectServer.config.key, host: config.ServerConfigs.host, port: config.ServerConfigs.port, peerhost: config.ServerConfigs.peerhost, peerport: config.ServerConfigs.peerport, secure: mode === 'DEV' ? false : true }
        });
      }
