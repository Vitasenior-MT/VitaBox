...
var ConnectServer = require('./connectServer'),
  ...
  reconnectRate = utils.timeCalculator(reconnectRate.hour, reconnectRate.min, reconnectRate.sec),
  utils = require('./utils.js'),
  index = 0,
  ...
  getUsers: (req, res) => {
    ConnectServer.get((data) => {
      ...
      if (data.responce) {
        ...
        console.log(config)
        console.log(ConnectServer.config.key)
        return res.json({
          status: true,
          data: { data: data.responce, vitaboxId: config.ServerConfigs.key, token: ConnectServer.config.key, host: config.ServerConfigs.host, port: config.ServerConfigs.port, peerhost: config.ServerConfigs.peerhost, peerport: config.ServerConfigs.peerport, secure: mode === 'DEV' ? true : false }
        });
      }
    }, '/vitabox/' + ConnectServer.config.key + '/user');
  },
  getTempo: (req, res) => {
  ...
  handler: (callback, result) => {
    if (result.status !== 200) {
      setTimeout(() => {
        ...
        index++;
        if (index > 10) {
          reconnectRate = utils.timeCalculator(reconnectRate.extra.hour, reconnectRate.extra.min, reconnectRate.extra.sec);
        }
      }, reconnectRate);
    } else {
      callback({
