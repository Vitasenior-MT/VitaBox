...
var self = module.exports = {
  ...
  getPatients: (res) => {
    console.log('getPatients ------- ', ConnectServer.config.key);
    ConnectServer.get((data) => {
  ...
  getTempo: (req, res) => {
    ConnectServer.get((data) => {
      self.handler((code) => {
        if (code === 'newPost') {
          self.getTempo(req, res);
        } else {
          let dataDL = JSON.parse(data.responce);
          // let dataDL = {};
          // dataDL.district = "santarem";
          // dataDL.locality = "tomar";
          let district = utils.normalyzeStringLowercase(dataDL.district)
          let locality = utils.normalyzeStringLowercase(dataDL.locality)
          let requestOptions = {
            encoding: null,
            method: "GET",
  ...
  getFarmaciasServico: (req, res) => {
    ConnectServer.get((data) => {
      self.handler((code) => {
        if (code === 'newPost') {
          self.getFarmaciasServico(req, res);
        } else {
          let dataDL = JSON.parse(data.responce);
          // let dataDL = {};
          // dataDL.district = "santarem";
          ...
          request(requestOptions, function (error, response, body) {
            let cnt = -1,
              ...
              resultFarmacias = {
                farmacias: [],
                district: dataDL.district.charAt(0).toUpperCase() + dataDL.district.substring(1),
                locality: dataDL.locality.charAt(0).toUpperCase() + dataDL.locality.substring(1),
                info: []
              };
            if (error) {
              return res.json({
