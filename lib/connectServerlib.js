  ...
  getUsers: (req, res) => {
    ConnectServer.get((data) => {
      if (data.data) {
        return res.json({
          status: true,
          data: { data: data.data.responce, vitaboxId: config.ServerConfigs.key, token: ConnectServer.config.key, host: config.ServerConfigs.host, port: config.ServerConfigs.port, secure: mode === 'DEV' ? true : false }
        });
      }
