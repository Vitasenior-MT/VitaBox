  ...
  getTempo: (req, res) => {
    if (process.env.connection) {
      settinglib.getLocationDistrict(null, (data) => {
        ...
        let district = data[0].district ? utils.normalyzeStringLowercase(data[0].district) : '';
        let locality = data[0].locality ? utils.normalyzeStringLowercase(data[0].locality) : '';
        let requestOptions = {
          encoding: null,
          method: "GET",
