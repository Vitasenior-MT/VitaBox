...
monitor.once(CECMonitor.EVENTS._READY, function() {
  ...
  console.log( ' -- READY -- ' );
  if (sckOn) {
    sckOn.sockets.emit('msgOutput', ' -- READY -- ');
}
  // rl.prompt();
});
...
monitor.on(CECMonitor.EVENTS._STOP, function () {
  ...
  if (sckOn) {
 sckOn.sockets.emit('msgOutput', 'cec-client exit');
}
  // rl.prompt();
})
...
monitor.on(CECMonitor.EVENTS._DEBUG, function(data) {
  ...
  if (sckOn) {
      sckOn.sockets.emit('msgOutput', '[DEBUG] '+JSON.stringify(data));
    }
  // rl.prompt();
})
...
function displayPayload(packet) {
  ...
  if (sckOn) {
    sckOn.sockets.emit('msgOutput', JSON.stringify(packet));
}
  // rl.prompt();
}
