...
var warninglib = require('./warninglib.js');
...
ServerSktIo.prototype.init = function () {
    ...
    remotelib.getAllRemoteSettings();
    warninglib.getAllWarnings();

    sckOn = this.io;
    // Fired upon a connection
    this.io.on("connection", function (socket) {

        remoteKeys = remotelib.getResults();
        modalMsg = warninglib.getResults();
...
function keyEventProcessing(cmd) {
    ...
    switch (cmd) {
        case "ch_1":
            writeMessage('tx 4F:82:10:00', true, 'remote');
            break;
        case "ch_2":
            writeMessage('tx 4F:82:20:00', true, 'remote');
            break;
        case "hdmirequest":
            hdmiState();
            break;
...
function writeMessage(cmd, hdmiRequest, type) {
...
function hdmiState(type) {
    if (type) {
        try {
            ...
            sckOn.sockets.emit('hdmiCurrentStatePost', { 'PhysAddress': physAddress }, type);
        } catch (e) {
            sckOn.sockets.emit('hdmiCurrentStatePost', { 'PhysAddress': 0 }, type);
        }
    } else {
        try {
            var physAddress = (monitor.GetActiveSource() === null) ? 0 : monitor.GetActiveSource();
            sckOn.sockets.emit('hdmiCurrentStatePost', { 'PhysAddress': physAddress });
        } catch (e) {
            sckOn.sockets.emit('hdmiCurrentStatePost', { 'PhysAddress': 0 });
        }
    }
}
