...
ServerSktIo.prototype.init = function () {
    ...
    this.io.on("connection", function (socket) {
        ...
        socket.on('ttsDelete', function () {
            exec('rm -Rf public/dist/static/.temp/*');
        });
...
function keyEvents(code) {
    remotelib.getKey(code, function (err, result) {
        if (code && result) {
            ...
            console.log("tecla não configurada na base de dados".red);
        }
    });
    ...
    if (!messageConfirmed) {
        ...
        switch (cmd) {
            ...
            case "menu":    // tecla alterar menu' ou tecla 'q'
                ...
                break;
            /*case "left":
                sendOutPutMsg('controllForm', "left");
                break;
            case "right":
                sendOutPutMsg('controllForm', "right");
                break;
            case "up":
                sendOutPutMsg('changeMenu', -1);
                break;
            case "down":
                sendOutPutMsg('changeMenu', 1);
                break;
