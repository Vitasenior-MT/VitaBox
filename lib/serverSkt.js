  ...
  functions = {
    ...
    address: function () {
      try {
        ...
        errorLog.error(`Error : ${e.toString()}`);
      }
    },
    active: function () {
...
ServerSktIo.prototype.init = function () {
  ...
  this.io.on("connection", function (socket) {
    ...
    socket.on('checkBleBlock', function () {
      checkMiBand((data) => {
        socket.emit('responceBleBlock', data);
      });
    })
    ...
    socket.on('ttsTextSteps', function (text) {
      successlog.info(`Text To Speech: ${text}`);
      audioEngine.getData(text, (path) => {
        socket.emit('ttsPathSteps', path);
      });
    });
    ...
    socket.on('requestWifiStatus', function () {
      verifyConnetion();
    });
    ...
    socket.on('terminal', function (cmd) {
      let cmdIO = exec(cmd); //comando a ser executado em shell
      cmdIO.stdout.on('data', (data) => { // captura toda a informação do comando executado
        successlog.info(`Command executed: ${data}`);
        socket.emit('recieve-data', data);
      });

      cmdIO.stderr.on('data', (data) => { // captura a informação de problemas que acontecem no processo 
        errorLog.error(`Could Not execute command: ${data}`);
        socket.emit('recieve-data', data);
      });
    });
  ...
  wifi.ping((data) => {
    if (data) {
      ...
      if (parseInt(data.loss) <= 20) {
        ...
        sendOutPutMsg('WIFI', true);
      } else {
        successlog.info(`Connect again`);
        sendOutPutMsg('WIFI', false);
...
function checkMiBand(callback) {
  let checkProcess = exec('ps aux | grep startAutoMiBand | awk \'{print $2}\''); //comando a ser executado em shell
  checkProcess.stdout.on('data', (data) => { // captura toda a informação do do processo
    let index = data.split('\n').length
    successlog.info(`Checking Mi band process: ${index}`);
    callback(index > 3 ? true : false);
  });

  checkProcess.stderr.on('data', (data) => { // captura a informação de problemas que acontecem no processo 
    errorLog.error(`Could Not Restart Cec: ${data}`);
    callback('Error');
  });
}
