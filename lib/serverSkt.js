...
var NodeCEC = require('./nodecec.js'),
  ...
  interfaceReset = utils.timeCalculator(0, 1, 0), // In case the configs fail the time wont be messed up
...
var ServerSktIo = function (options) {
  ...
  interfaceReset = utils.timeCalculator(this.TimersConfig.interfaceReset.hour, this.TimersConfig.interfaceReset.min, this.TimersConfig.interfaceReset.sec);
...
function KeyCMD(cmd) {
  switch (cmd) {
    case "channel":
      ...
      break;
    case "message":
      serverBoardManager.buildObject('60ab', 'mono', '600', serverBoardManager.sensor_list)
      /*blinkMessage(type, {
        type: 'notification',
...
function verifyConnetion() {
  let number = 5, site = 'www.google.com';
  wifi.ping((data) => {
    successlog.info(`Data ${data.recieved} ${data.loss}`);
    if (data.recieved === number && data.loss === '0%') {
