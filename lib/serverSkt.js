...
var NodeCEC = require('./nodecec.js'),
    ...
    functions = {
        ...
        address: function() {
            ...
            sendOutPutMsg('My address:');
            sendOutPutMsg(lpad('logical addresses: ', 19) + monitor.GetLogicalAddresses().join(', '));
            sendOutPutMsg(lpad('primary logical: ', 19) + monitor.GetLogicalAddress());
            sendOutPutMsg(lpad('physical: ', 19) + monitor.GetPhysicalAddress());
        },
        active: function() {
            console.log('active source: ' + monitor.GetActiveSource());
            sendOutPutMsg('active source: ' + monitor.GetActiveSource());
        },
        logical: function(physical) {
            console.log('primary logical address: ' + monitor.Physical2Logical(physical));
            sendOutPutMsg('primary logical address: ' + monitor.Physical2Logical(physical));
        },
        physical: function(logical) {
            console.log('physical address: ' + monitor.Logical2Physical(logical));
            sendOutPutMsg('physical address: ' + monitor.Logical2Physical(logical));
        },
        power: function(address) {
            console.log('power status: ' + monitor.GetPowerStatusName(address) + ' (' + monitor.GetPowerStatus(address) + ')');
            sendOutPutMsg('power status: ' + monitor.GetPowerStatusName(address) + ' (' + monitor.GetPowerStatus(address) + ')');
        },
        osdname: function(address) {
            console.log('OSD name: ' + monitor.GetOSDName(address));
            sendOutPutMsg('OSD name: ' + monitor.GetOSDName(address));
        },
        state: function() {
        ...
        cp: function(address) {
            monitor.SendCommand(null, address, CEC.Opcode.GIVE_DEVICE_POWER_STATUS, CECMonitor.EVENTS.REPORT_POWER_STATUS)
                .then(function(packet) {
                    ...
                    sendOutPutMsg('POWER', packet.data.str);
                })
                .catch(function(error) {
                    console.log('POWER UNKNOWN');
                    console.log(error);
                    sendOutPutMsg('POWER UNKNOWN');
                    sendOutPutMsg(error);
                })
        },
        hdmi: function(port) {
...
ServerSktIo.prototype.init = function() {
    ...
    this.io.on("connection", function(socket) {
        ...
        socket.on("runTxCmd",function(txcmd){
            monitor.WriteRawMessage('tx ' + txcmd.toLowerCase().trim().replace("tx ", ""));
        });

        socket.on("rumSendMsg", function(data){
            // console.log("Msg", data);
            var msg = data.msg.toString().trim();
            var msg2 = "";
            for (var i = 0; i < msg.length; i++){
                msg2 += msg.charCodeAt(i).toString(16) + ":";
            } 
            msg2 = msg2.slice(0, msg2.length - 1);
            // console.log("Send msg", 'tx ' + monitor.GetPhysicalAddress().split(".")[0] + "0:64:" + data.optmsg + ":" +  msg2);
            monitor.WriteRawMessage('tx ' + monitor.GetPhysicalAddress().split(".")[0] + "0:64:" + data.optmsg + ":" +  msg2);
        });

        socket.on('execCommandCEC', function(cmd) {
            console.log('Command: ' + cmd);
            var commandCEC = cmd.toString().toLowerCase().replace("tx ", "").substring(1);
            // console.log('command: ' + 'tx ' + monitor.GetPhysicalAddress().split(".")[0] + commandCEC);

            monitor.WriteRawMessage('tx ' + monitor.GetPhysicalAddress().split(".")[0] + commandCEC);
        });
...
function sendOutPutMsg(msg){
    if (sckOn) {
        sckOn.sockets.emit('msgOutput', msg);
    }    
}

function displayPayload(packet) {
  console.log(JSON.stringify(packet));  
  sendOutPutMsg(packet);
  // rl.prompt();
}
