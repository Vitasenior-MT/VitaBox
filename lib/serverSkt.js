...
monitor.once(CECMonitor.EVENTS._READY, function() {
  functions.address();
  console.log( ' -- READY -- ' );
 this.io.sockets.emit('msgOutput', ' -- READY -- ');
  // rl.prompt();
});
...
monitor.on(CECMonitor.EVENTS._STOP, function () {
 ...
 this.io.sockets.emit('msgOutput', 'cec-client exit');
  // rl.prompt();
})
...
monitor.on(CECMonitor.EVENTS._DEBUG, function(data) {
  ...
  this.io.sockets.emit('msgOutput', '[DEBUG] '+JSON.stringify(data));
  // rl.prompt();
})
...
function displayPayload(packet) {
 ...
 this.io.sockets.emit('msgOutput', JSON.stringify(packet));
  // rl.prompt();
}
