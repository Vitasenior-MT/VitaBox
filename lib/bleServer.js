    ...
    if (data) {
      ...
      utilsBle.nobleDeviceReturnService(device, ["1809"], function (service) {
        ...
        utilsBle.nobleDeviceReturnCaract(service, ["2a1e"], function (characteristictemp) {
          /**
           * TODO: Descoberta do descritor da caracteristica
           */
                  ...
                  let tempCalc = (function () {
                    ...
                    if (tempMedia.length === 5) {
                      for (var i = 0; i < tempMedia.length; i++) {
                        calcT += tempMedia[i]; //don't forget to add the base
                      }
                    }
                    return Math.round((calcT / tempMedia.length) * 100) / 100;
                  })();

                  maxSnrTestCount--;
                  if (maxSnrTestCount >= 0 && tempCalc != temp) {
                    tempSum = temp
                    console.log("Body Temperature, nova leitura.", (maxSnrTest - maxSnrTestCount), tempCalc, temp);
                    sleep.sleep(5);
                    let command = utilsBle.dataCommand([0x01, 0x00]);
                    utilsBle.nobleDescriptorWriteCommand(descriptor, command, function () {
                    });
                    ...
                    if (!sendValuesDB) {
                      let newDataSensors = [];
                      newDataSensors["bodytemp"] = temp; // Math.round((tempSum / maxSnrTest) * 100) / 100;
                      console.log("Body Temperature.", newDataSensors["bodytemp"]);
                      sendValuesDB = true;
                      /**
                       * TODO: Envia os dados para a base de dados e para a interface
                       */
        ...
        characteristic.on('read', function (data, isNotification) {
          ...
          if (data && isNotification) {
            ...
            for (var ii = 0; ii < data.length; ii++) {
              str += data[ii].toString(10) + ', ';
            };
            console.log(str);

            maxSnrTestCount--;
            if ((
              (pulse_spo2 * 1) === 127 || (spo2 * 1) === 127 ||
              (spo2 * 1) < 80 || (pulse_spo2 * 1) < 30) && maxSnrTestCount >= 0) {
              ...
              sleep.sleep(1);
              command = utilsBle.dataCommand([0x01]);
              utilsBle.nobleDescriptorWriteCommand(descriptor, command, function () {
              });     
              sleep.sleep(4);
              console.log("Dados incorrectos, nova tentativa.");
