  ...
  utilsBle.nobleDeviceReturnDiscriptor(characteristic, '2902', function (descriptor) {
    ...
    utilsBle.nonleDescriptorWriteCommand(descriptor, command, function () {
      ...
      if (!self.options.bandfitness_auth) {
        ...
        console.log(' Sending Key... ' + secret_key.toString("hex"));

      } else {
        secret_key = utilsBle.dataCommand([0x02, 0x08]);
        console.log(' Sending Key if auth... ' + secret_key.toString("hex"));
      }
      ...
      utilsBle.nobleCaracteristicWrite(characteristic, secret_key, true, function () {
        ...
        characteristic.on('read', function (data, isNotification) {
          ...
          if (data.includes(aux1)) {
            ...
            console.log(' Random number received... ' + rand_number.toString("hex") + ' len: ' + rand_number.length);
            ...
            console.log(' Sending encrypted random number... ' + send_cmd.toString("hex"));
            //enviar tudo
            ...
            utilsBle.nonleDescriptorWriteCommand(descriptor, command, function () {
              ...
              // self.bandfitnessDeviceExec(device, ["heartrate"]);
            });
            ...
            console.log(' Auth error, cant help! Response:  ' + data.toString("hex"));
...
BleServer.prototype.bandfitnessHardwareRevision = function (device, service, characteristic, others) {
  ...
  characteristic.read(function (error, data) {
    if (data) {
      var HardwareRevisionString = data;
...
BleServer.prototype.bandfitnessSoftwareRevision = function (device, service, characteristic, others) {
  ...
  characteristic.read(function (error, data) {
    if (data) {
      var SoftwareRevisionString = data;
...
BleServer.prototype.bandfitnessTime = function (device, service, characteristic, others) {
  ...
  characteristic.read(function (error, data) {
    if (data) {
      var TimeInfo = data;
      var year = TimeInfo[1] + 2011;
      var month = TimeInfo[2];
      var day = TimeInfo[3];
      var hour = TimeInfo[4] + 1;
      var minute = TimeInfo[5];
      var second = TimeInfo[6];
  ...
  characteristic.read(function (error, data) {
    if (data) {
      ...
      let dataSend = {
        ...
        exec: "batterystatus",
        data: {
          battery_level: BatteryInfo[1],
          last_time_full: (BatteryInfo[6] + '/' + BatteryInfo[5] + '/' + (BatteryInfo[4] + 2011) + ' at ' + BatteryInfo[7] + 'h' + BatteryInfo[8] + ':' + BatteryInfo[9]),
          last_time_charged: (BatteryInfo[14] + '/' + BatteryInfo[13] + '/' + (BatteryInfo[12] + 2011) + ' at ' + BatteryInfo[15] + 'h' + BatteryInfo[16] + ':' + BatteryInfo[17]),
...
BleServer.prototype.bandfitnessSteps = function (device, service, characteristic, others) {
  ...
  characteristic.read(function (error, data) {
    if (data) {
      var StepsInfo = data;
      // console.log('  Steps : ' + StepsInfo[1] + '' + StepsInfo[2] + '' + StepsInfo[3]);
      // console.log('  Meters : ' + StepsInfo[5] + '' + StepsInfo[6] + '' + StepsInfo[7]);
      // console.log('  Callories : ' + +StepsInfo[9]);
      let dataSend = {
        status: true,
        exec: "Steps",
        data: {
          Steps: (StepsInfo[1] + '' + StepsInfo[2] + '' + StepsInfo[3]),
          meters: (StepsInfo[5] + '' + StepsInfo[6] + '' + StepsInfo[7]),
          callories: StepsInfo[9]
        }
      };
      utilsBle.sendMsgCommand(true, "bleExecFimBandFitness", dataSend, false);
      if (others.length > 0) {
...
BleServer.prototype.bandfitnessHeartRate = function (device, service, characteristic, others) {
  var self = this;
  //flag para vibração da miband
  var vibrateflag = 0;
  //variável para verificar se já efetuou 5 medições da pulsação
  var contVibrate = 0;

  (function checkVibrateFlag() {
    if (contVibrate > 2) {
      //depois de vibrar 3 vezes, caracteristicas para a pulsação (2a37 - notificações ; 2a39 - pedidos)
      let opts = ['180d', '2a37'];
      let nextExec = 'bandfitnessHeartRateGetData';
      self.deviceDefaultDiscover(device, opts, nextExec);
    } else {
      if (vibrateflag == 0) {
        console.log(' Vibrate Caracteristic  --- ON');

        utilsBle.sendMsgCommand(true, "bleMsg", "Activar a vibração.", false);
        var vibrateON = utilsBle.dataCommand([0x02]);
        //enviar comando para fazer vibrar a miband
        utilsBle.nobleCaracteristicWrite(characteristic, vibrateON, true, function () {
          vibrateflag = 1;
          sleep.sleep(2);
          checkVibrateFlag();
        });
      } else {
        console.log(' Vibrate Caracteristic  --- OFF');
        utilsBle.sendMsgCommand(true, "bleMsg", "Desactivar a vibração.", false);
        var vibrateOFF = utilsBle.dataCommand([0x00]);
        //enviar comando para fazer parar a vibração da miband
        utilsBle.nobleCaracteristicWrite(characteristic, vibrateOFF, true, function () {
          vibrateflag = 0;
          contVibrate++;
          sleep.sleep(2);
          checkVibrateFlag();
...
BleServer.prototype.bandfitnessHeartRateGetData = function (device, service, characteristic, others) {
  var self = this;
  ...
  utilsBle.nobleDeviceReturnDiscriptor(characteristic, '2902', function (descriptor) {
    ...
    utilsBle.nonleDescriptorWriteCommand(descriptor, hr_notifications, function () {
      ...
      utilsBle.nobleDeviceReturnCaract(service, '2a39', function (heartratecontrolcaracteristic) {
        ...
        utilsBle.sendMsgCommand(true, "bleMsg", "Inicioar o processo de leitura da pulsação.", false);
        utilsBle.nobleCaracteristicWrite(heartratecontrolcaracteristic, start_Heart_Rate, false, function () {
          ...
          characteristic.on('data', function (data, isNotification) {
            clearTimeout(noDevice);
            console.log("Note", data.readInt8(0), isNotification);
            if (data && isNotification) {
              console.log(' HEART RATE ' + (contHR++) + ' : ' + data.readInt8(0) + data.readInt8(1));
              let dataSend = {
                status: true,
                exec: "heartrate",
                data: {
