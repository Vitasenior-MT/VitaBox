      ...
      utilsBle.nobleCaracteristicRead(characteristic, function () {
        characteristic.on('read', function (data, isNotification) {
          ...
          if (data && isNotification) {
            ...
            if (((newDataSensors["pulse"] * 1) === 127 || (newDataSensors["spo2"] * 1) === 127) && maxSnrTestCount > 0) {              
              return self.bodypulseCollectData(device, service, characteristic, descriptor);
            } else if (maxSnrTestCount === 0) {
              return utilsBle.sendMsgCommand(true, "bleError", "Erro ao adquirir os dados. Tente novamente.", true);
            } else {
              if (!sendValuesDB) {
                sendValuesDB = true;
                /**
