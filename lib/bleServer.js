      ...
      */
      utilsBle.nobleDeviceReturnService(device, ["fff3"], function (service) {
        ...
        utilsBle.nobleDeviceReturnCaract(service, ["fff4"], function (characteristicgluco) {
           ...
           * TODO: Descoberta do descritor da caracteristic
           */
          utilsBle.nobleDeviceReturnDiscriptor(characteristicgluco, '2902', function (descriptor) {
            /**
             * TODO: Criação para comando para ativar o descritor
             */
            console.log(' Enabling Glucometer notifications status... ');
            let command = utilsBle.dataCommand([0x01, 0x00]);
            /**
             * TODO: Envia o comando para o descritor
             */
              ...
              utilsBle.nobleDeviceReturnService(device, ["fff0"], function (service) {
                ...
                utilsBle.nobleDeviceReturnCaract(service, ["fff1"], function (characteristic) {
                  ...
                  utilsBle.nobleDeviceReturnDiscriptor(characteristic, '2902', function (descriptor) {
                     ...
                     * TODO: Criação para comando para ativar o descritor
                     */
                    console.log(' Enabling Glucometer2 notifications status... ');
                    let command = utilsBle.dataCommand([0x01, 0x00]);
                    /**
                     * TODO: Envia o comando para o descritor
                     */
                    utilsBle.nonleDescriptorWriteCommand(descriptor, command, function () {
                      ...
                      characteristic.on('read', function (data, isNotification) {
                        ...
                        if (data && isNotification) {
                          ...
                          var aux1 = Buffer.from([0x0c]);
                          var aux2 = Buffer.from([0x0d]);
                          var aux3 = Buffer.from([0x0e]);
                          var aux4 = Buffer.from([0x4c]);
                          var aux5 = Buffer.from([0x48]);
                          if (data[8] == aux1) {
                            var gh = data.readInt8(12) - 48;
                            var gl = data.readInt8(13) - 48;
                            glucose = (gh * 10) + gl;
                            info = 0;
                            console.log("Glucose(mg/dl): " + glucose);
                          }
                          if (data[8] == aux2) {
                            var gh = data.readInt8(12) - 48;
                            var gm = data.readInt8(13) - 48;
                            var gl = data.readInt8(14) - 48;
                            glucose = (gh * 100) + (gm * 10) + gl;
                            info = 0;
                            console.log("Glucose(mg/dl): " + glucose);
                          }
                          if (data[8] == aux3) {
                            if (data[12] == aux4) {
                              glucose = 0;
                              info = 0xAA;
                              console.log("Low glucose");
                            } else if (data[12] = aux5) {
                              glucose = 360;
                              info = 0xBB;
                              console.log("High glucose");
                            }
                          }
                          let newDataSensors = [];
...
BleServer.prototype.bodypulse = function (device, service, characteristic) {
  ...
  utilsBle.nobleDeviceReturnDiscriptor(characteristic, '2902', function (descriptor) {
    ...
    self.bodypulseLoopAvg(device, characteristic, descriptor);
  });
}

BleServer.prototype.bodypulseLoopAvg = function (device, characteristic, descriptor) {
  var self = this;
    ...
    utilsBle.nobleCaracteristicRead(characteristic, function () {
      characteristic.on('read', function (data, isNotification) {
        ...
        if (data && isNotification) {
          ...
          let pulse_low = data.readInt8(11);
          pulse_low &= 0b01111111;

          let pulse_high = data.readInt8(10);
          ...
          let spo2 = data.readInt8(13);
          spo2 &= 0b01111111;

          // console.log("SpO2: " + spo2 + "%");
          // console.log("Pulse: " + pulse_spo2 + "ppm");

          if (spo2LoopControl > 0) {
            spo2LoopControl--;
            pulse_spo2Avg += pulse_spo2;
            spo2Avg += spo2;
            setTimeout(() => {
              self.bodypulseLoopAvg(device, characteristic, descriptor);
            }, 5000);            
            ...
            let newDataSensors = [];
            newDataSensors["spo2"] = (pulse_spo2Avg / spo2LoopMax);
            newDataSensors["pulse"] = (spo2Avg / spo2LoopMax);

            console.log("Send SpO2: " + newDataSensors["spo2"]  + "%");
            console.log("Send Pulse: " + newDataSensors["pulse"] + "ppm");
            /**
             * TODO: Envia os dados para a base de dados e para a interface
             */
