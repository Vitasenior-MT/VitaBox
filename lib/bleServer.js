    ...
    utilsBle.nobleCaracteristicRead(characteristic, function () {
      characteristic.on('read', function (data, isNotification) {
        ...
        if (data && isNotification) {
          ...
          if (
            (
              (newDataSensors["pulse"] * 1) === 127 || 
              (newDataSensors["spo2"] * 1) === 127 || 
              (newDataSensors["spo2"] * 1) <= 80 || 
              (newDataSensors["pulse"] * 1) < 30) && maxSnrTestCount >= 0 && !sendValuesDB && Math.abs(maxSnrTestCount) < maxSnrTest) {   
