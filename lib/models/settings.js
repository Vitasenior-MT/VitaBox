...
var SettingSchema = new Schema({
  ...
  flg_mdevice: { type: Boolean, default: false },
  flg_screen: { type: Boolean, default: false },
...
Setting.prototype.updateFlagBandFit = function (flg, res) {
  console.log(flg);
  var query = { id: 1 },
    options = { upsert: true, new: true, setDefaultsOnInsert: true },
    update = {
...
Setting.prototype.updateDeviceType = function (type, res) {
  console.log(type);
  var query = { id: 1 },
    options = { upsert: true, new: true, setDefaultsOnInsert: true },
    update = {
  ...
  this.settingdb.findOneAndUpdate(query, update, options, (err, result) => {
    sendData(res, err, 'deviceType: ' + result.deviceType);
  });
}

Setting.prototype.updateSFlgMDevice = function (flg, res) {
  console.log(flg);
  var query = { id: 1 },
    options = { upsert: true, new: true, setDefaultsOnInsert: true },
    update = {
  ...
  this.settingdb.findOneAndUpdate(query, update, options, (err, result) => {
    sendData(res, err, 'flg_mdevice: ' + result.flg_mdevice);
  });
}
...
Setting.prototype.getFlgData = function (res) {
  this.settingdb.find({}, {
    _id: 0,
    flg_bandfitness: 1,
    deviceType: 1,
...
Setting.prototype.updateFlgScreen = function (flg, res) {
  console.log(flg);
  var query = { id: 1 },
    options = { upsert: true, new: true, setDefaultsOnInsert: true },
    update = {
  ...
  this.settingdb.findOneAndUpdate(query, update, options, (err, result) => {
    sendData(res, err, 'flg_screen: ' + result.flg_screen);
  });
}

Setting.prototype.getFlgScreen = function (res) {
  this.settingdb.find({}, {
    _id: 0,
    flg_screen: 1,
  }, function (err, result) {
