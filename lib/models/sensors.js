...
Sensor.prototype.create = function (boards) {
  ...
  for (let index = 0; index < boards.boards.length; index++) {
    ...
    for (let i = 0; i < board.Sensors.Sensormodel.length; i++) {
      sensors = {
        board_id: board.id,
        node_id: board.node_id,
        location: board.location,
        sensor_id: board.sensors.Sensormodel[i].id,
        sensortype: board.sensors.Sensormodel[i].tag,
        threshold_min_acceptable: board.sensors.Sensormodel[i].min_acceptable,
        threshold_max_acceptable: board.sensors.Sensormodel[i].max_acceptable,
        threshold_min_possible: board.sensors.Sensormodel[i].min_possible,
        threshold_max_possible: board.sensors.Sensormodel[i].max_possible
      };
 ...
 */
Sensor.prototype.update = function (data, res, functions, callback) {
  var self = this;
  data.critLevel = findCritLvl(data.avg, data.threshold_max_possible, data.threshold_max_acceptable);
  ...
  this.sensordb.update({ 'board_id': data.board_id, "sensor_id": data.sensor_id },
    {
      ...
      functions.sendMsgToPage("avgSensorUpdate", {
        id: data.board_id,
...
/**
 * TODO: Creates the critical level of the sensor
 * @param { Average of the sensor data } avg
 * @param { Maximum value possible of the threshold } max_possible
 * @param { Maximum value acceptable of the threshold } max_acceptable
 */
