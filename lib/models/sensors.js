...
Sensor.prototype.agragateSensorData = function (sensortype, id, location) {
  ...
  this.sensordb.aggregate(query, (err, result) => {
    ...
    if (result) {
      self.insertOrUpdateAvg(id, sensortype, result[0].avg);
...
Sensor.prototype.insertOrUpdateAvg = function (idsensor, sensortype, avg) {
  this.sensordb.update({ "id": idsensor, "values.sensortype": sensortype },
    { $set: { "values.$.avg": avg } }, function (err, result) {
      if (err) {
...
Sensor.prototype.getAllCriticalSensorses = function (res) {
  this.sensordb.aggregate([
    {
      $unwind: '$values'
    }, {
      $match: {
        'values.avg': { $gte: 'values.threshold' }
      }
    }, {
      $project: {
        id: '$id',
