...
Sensor.prototype.getSensorData = function (res) {
  var self = this;
  this.sensordb.find({}, function (err, result) {
    console.log('***************************Start***************************');
    console.log(result);
    var saveData = [];
    self.getData(res, result.length - 1, result[result.length - 1].values.length - 1, result, saveData);
  });
};

Sensor.prototype.getData = function (res, index, i, arr, saveData) {
  var self = this;
  var query = [{
    $unwind: '$values'
  }, {
  ...
  this.sensordb.aggregate(query, function (err, result) {
    ...
    console.log('index, i, arr, saveData');
    console.log(index);
    console.log(i);
    ...
    if (index === 0) {
      if (i === 0) {
        ...
        self.getData(res, index, --i, arr, saveData);
      }
    } else {
      if (i === 0) {
        var j = --index;
        self.getData(res, j, arr[j].values.length - 1, arr, saveData);
      } else {
        self.getData(res, index, --i, arr, saveData);
      }
    }
...
Array.prototype.findByID = function (id) {
  var i = this.length;
  while (i--) {
    var j = this[i].values.length;
    while (j--) {
      var w = this[i].values[j].value.length;
      while (w--) {
        if (this[i].values[j].value[w].id === id) {
          this[i].values[j].value[w].flg_available = false;
        }
      }
    }
  }
