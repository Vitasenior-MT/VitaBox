...
Sensor.prototype.sensorVerify = function (id, board) {
  ...
  this.sensordb.findOne({ id: id }, (err, result) => {
    if (board.Boardmodel.Sensors.length > 0) {
      let sensor = board.Boardmodel.Sensors;
      for (var i = 0; i < sensor.length; i++) {
        if (!result.values.verify(sensor[i].id)) {
          result.values.push({
            sensortype: sensor[i].tag,
            threshold_flg: board.Boardmodel === 'environmental' ? true : false,
            threshold: {
...
Array.prototype.verify = function (id) {
  var i = this.length;
  while (i--) return id === this[i].id ? true : false;
  return false;
}
