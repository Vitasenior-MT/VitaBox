...
var mongoose = require('mongoose'),
  ...
  execludeAmbientSensors = require('./../../config-' + mode.toLowerCase() + '/config.js').execludeAmbientSensors,
...
var SensorSchema = new Schema({
  ...
  threshold_min_graph: { type: Number, required: false },
  threshold_max_graph: { type: Number, required: false },
      ...
      for (let i = 0; i < board.Sensors.length; i++) {
        if (board.Boardmodel.type === "environmental") {
          sensors = {
            ...
            threshold_min_acceptable: board.Sensors[i].Sensormodel.min_acceptable || 0,
            threshold_max_acceptable: board.Sensors[i].Sensormodel.max_acceptable || 0,
            threshold_min_possible: board.Sensors[i].Sensormodel.min_possible || 0,
            threshold_max_possible: board.Sensors[i].Sensormodel.max_possible || 0,
            threshold_min_graph: board.Sensors[i].Sensormodel.min_graph || 0,
            threshold_max_graph: board.Sensors[i].Sensormodel.max_graph || 0
          };
 ...
 */
Sensor.prototype.update = function (warning, data, functions) {
  ...
  this.sensordb.update({ 'board_id': data.board_id, "sensor_id": data.sensor_id },
    {
      ...
      if (warning) {
        if (excludeEnviromentalSensors(data.sensortype)) {
 ...
 * @param { Is invoked after the query } callback
 */
Sensor.prototype.getSensorsInfo = function (location, res) {
...
Sensor.prototype.getSensorsWithLocation = function (data, res) {
  this.sensordb.aggregate([
    {
      $match: {
        'location': data.location
 ...
 * @param { Is invoked after the query } res
 */
Sensor.prototype.getThresholds = function (data, res) {
  this.sensordb.aggregate([
    {
      $match: {
        'location': data.location
...
Sensor.prototype.getDistictAllLocations = function (res) {
  var query = [{
    $group: {
      _id: null,
      locations: {
  ...
  for (var index = 0; index < obj.length; index++) {
    ...
    if (!sensors[obj[index].node_id].sensorID[obj[index].sensortype]) {
      sensors[obj[index].node_id].sensorID[obj[index].sensortype] = {
        ...
        threshold_min_graph: obj[index].threshold_min_graph,
        threshold_max_graph: obj[index].threshold_max_graph,
        avg: obj[index].avg,
...
/**
 * TODO: checks for the same sensor type
 * @param { String } sensortype the type of sensor in string
 */
var excludeEnviromentalSensors = function (sensortype) {
  var excludeData = execludeAmbientSensors.split('&&');
  for (var index in excludeData) {
    if (excludeData[index] === sensortype) {
      return false
    }
  }
  return true;
