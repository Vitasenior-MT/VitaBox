...
RawSensor.prototype.getDataToPost = function (callback) {
  this.rawsensordb.find({ flg_available: true }).limit(100).sort({ datetime: 1 }).exec((err, result) => {
    if (err) {
      return console.log("Error", err);
    }
    callback(result);
...
RawSensor.prototype.RawSensor = function(id, res){
/**
 db.rawsensors.aggregate({
  $match: { id: '02d3468c-d8b7-4895-a678-71f2efa715dd-00:a0:50:04:26:2e' }
}, {
  $sort: { datetime: -1 }
}, {
  $group: {
