 ...
 */
RawSensor.prototype.insert = function (data, sendData, callback) {
  this.rawsensordb.insertMany(data, function (err, result) {
    if (err) {
      return console.log("Error", err);
    }
...
RawSensor.prototype.getDataToPost = function (callback) {
  this.rawsensordb.find({ flg_available: true }, { value: 1, datetime: 1, _id: 1, sensor_id: 1 }).limit(100).sort({ datetime: 1 }).exec((err, result) => {
    if (err) {
      return console.log("Error", err);
    }
    callback(result);
