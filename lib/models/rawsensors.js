...
RawSensor.prototype.getSensorsByIdLastData = function (id, action, res, callback) {
  let query = [{
    $match: { id: id }
  }, {
    $sort: { datetime: -1 }
...
RawSensor.prototype.getSensorDataByTypeAndLocationV2 = function (dataSnr, type, location, res) {
  let query = [{
    $match: {
      transducer: type,
      location: location,
  ...
  this.rawsensordb.aggregate(query, function (err, result) {
    ...
    result[0].threshold_max_acceptable = dataSnr[0].threshold_max_acceptable;
    result[0].threshold_max_possible = dataSnr[0].threshold_max_possible;
    result[0].threshold_min_acceptable = dataSnr[0].threshold_min_acceptable;
    result[0].threshold_min_possible = dataSnr[0].threshold_min_possible;
...
/**
* @method postSensorData
* @description envia os dados para a cloud
* @param { Function } callback
*/
RawSensor.prototype.postSensorData = function (callback) {
  var self = this;
  let dataToSend = [];
  this.getDataToPost((result) => { // função para ir buscar os ultimos 100 registos
    if (result.length > 0) { // verifica se o resultado tem registos
      result.map((doc) => { dataToSend.push(doc); }); // insere os dados numa nova variavel
      connectServerlib.postSensorData(null, (data) => { // envia os dados para a cloud
        if (data.status === 200) {
          self.updateFlg(dataToSend, (modified) => { // caso tenha enviado os dados actualiza a flag dos registos enviados
            callback(true); // passa como argumento um boolean a true
          });
        }
      }, dataToSend);
    } else {
      callback(false); // passa como argumento um boolean a false
    }
  });
}
