 ...
 * @description Gets all the values from the table with flg_available false
 * @param { Is invoked after the query } res
 */
...
RawSensor.prototype.getSensorDataByLocation = function (location, res) {
  let query = [{
    $match: {
      location: location,
      bio: false,
  ...
  this.getDataToPost((result) => { // função para ir buscar os ultimos 100 registos
    if (result.length > 0) { // verifica se o resultado tem registos
      ...
      connectServerlib.postSensorData(null, (data) => { // envia os dados para a cloud
        console.log('Result from sending to the cloud: ', data);
        if (data.status === 200) {
