...
const customInfoLog = require('./logger').customInfoLog;
var configLogger = require('./logger').configLogger;
...
ServerBoardRegisterRevised.prototype.process = function (hostname, node_id) {
	...
	coap.request({
		...
		} else customInfoLog.info(`Empty: ${response}`);
	}).on('error', (coap_res) => customInfoLog.info(`Error: ${coap_res}, data: ${new Date(Date.now())}, hostname: ${hostname}, node_id: ${node_id}`)).end();
}
