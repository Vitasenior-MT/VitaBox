...
const commands = {
  ...
  rfkill: 'sudo rfkill unblock wifi; sudo rfkill unblock all',
...
/**
 * @method rfkillUnlock
 * @description unlocks all interfaces
 * @param {function} callback (err) Returns an error if the process fails
 */
function rfkillUnlock(callback) {
  exec(commands.rfkill, function (err) {
    callback(err);
  });
}
...
function interfaceDown(iface, callback) {
  iface = iface || currentInterface;
  console.log(replaceInCommand(commands.interfaceDown, { interface: iface }));
  exec(replaceInCommand(commands.interfaceDown, { interface: iface }), function (data, err) {
