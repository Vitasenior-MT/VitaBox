...
WifiManager.prototype.ping = function (callback) {
    var self = this;
    console.log('**********Ping**************');
    console.log(self.replaceInCommand(commands.ping, { site: 'www.google.com' }));

    let restartCec = exec(self.replaceInCommand(commands.ping, { site: 'www.google.com' })); //comando a ser executado em shell
    ...
    setTimeout(() => {
        console.log('********->**************');
        restartCec.on('exit', function () {
            console.log('********->**************');
        });
        ////restartCec.stdin.pause();
        //restartCec.kill();
    }, 2000);
}
