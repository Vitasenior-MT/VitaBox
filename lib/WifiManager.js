...
WifiManager.prototype.ping = function (callback, site) {
    var self = this;
    let restartCec = exec(self.replaceInCommand(commands.ping, { number: 3, site: site }));
    restartCec.stdout.on('data', (data) => {
        let lines = data.split(','), wifiData;
        if (lines.length > 2) {
            wifiData = {
                recieved: lines[1].split(' ')[1],
                loss: lines[2].split(' ')[1]
            }
            callback(wifiData);
        }
    });
